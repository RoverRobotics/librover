...
namespace RoverRobotics
{
    ...
    while (true)
    {
      ...
      else if (comm_type_ == "can")
      {
        ...
        for (int i = 0; i < 4; i++)
        {
          ...
          if (motors_speeds_[i] == 0 && robotstatus_.linear_vel == 0 && robotstatus_.angular_vel == 0)
          {
            v = 0;
            write_buffer = {
                i | 0x80000100U,
  ...
  void Pro2ProtocolObject::motors_control_loop(int sleeptime)
  {
    ...
    Control::robot_geometry robot_geometry = {0.4191, 0.46355, 0.1397, 0, 0};
    Control::pid_gains pid_gains = {0.0009, 0.0000, 0.00009};
