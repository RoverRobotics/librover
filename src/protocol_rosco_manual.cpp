...
RMProtocolObject::RMProtocolObject(
  ...
  rosco_control_ = std::make_unique<Control::RoscoManualController>(
  ...
  rosco_control_->setOpenLoopMaxRpm(OPEN_LOOP_MAX_RPM_);
  rosco_control_->setAngularScaling(angular_scaling_params_);
  ...
  switch (robot_mode_) {
    case Control::OPEN_LOOP:
      rosco_control_->setOperatingMode(Control::OPEN_LOOP);
      rosco_control_->setAccelerationLimits({std::numeric_limits<float>::max(),
                                            std::numeric_limits<float>::max()});
      break;
    case Control::TRACTION_CONTROL:
      rosco_control_->setOperatingMode(Control::TRACTION_CONTROL);
      rosco_control_->setAccelerationLimits(
          {LINEAR_JERK_LIMIT_, std::numeric_limits<float>::max()});
      break;
    case Control::INDEPENDENT_WHEEL:
      rosco_control_->setOperatingMode(Control::INDEPENDENT_WHEEL);
      rosco_control_->setAccelerationLimits(
          {LINEAR_JERK_LIMIT_, std::numeric_limits<float>::max()});
      break;
  }
...
void RMProtocolObject::load_persistent_params() { // RM is shorthand for RoscoManual
...
void RMProtocolObject::update_drivetrim(double delta) {
  ...
  if (-MAX_CURVATURE_CORRECTION_ < (trimvalue_ + delta) && (trimvalue_ + delta) < MAX_CURVATURE_CORRECTION_) {
    ...
    rosco_control_->setTrim(left_trim_, right_trim_);
...
void RMProtocolObject::send_estop(bool estop) {
  robotstatus_mutex_.lock();
  estop_ = estop;
  robotstatus_mutex_.unlock();
}

robotData RMProtocolObject::status_request() { 
  robotstatus_mutex_.lock();
...
robotData RMProtocolObject::info_request() { 
  robotstatus_mutex_.lock();
  auto returnData = robotstatus_;
  robotstatus_mutex_.unlock();
  return returnData; 
...
void RMProtocolObject::set_robot_velocity(double *control_array) {
  robotstatus_mutex_.lock();
  robotstatus_.cmd_linear_vel = control_array[0];
  robotstatus_.cmd_angular_vel = control_array[1];
  robotstatus_.cmd_left_hip_vel = control_array[3];
  robotstatus_.cmd_right_hip_vel = control_array[4];
  robotstatus_.cmd_ts = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::system_clock::now().time_since_epoch());
  robotstatus_mutex_.unlock();
}

void RMProtocolObject::unpack_comm_response(std::vector<uint8_t> robotmsg) {
...
bool RMProtocolObject::is_connected() { return comm_base_->is_connected(); }

void RMProtocolObject::register_comm_base(const char *device) {
...
void RMProtocolObject::send_command(int sleeptime) {
  while (true) {
    ...
    for (uint8_t vid = VESC_IDS::FRONT_LEFT; vid <= VESC_IDS::BACK_RIGHT;
      ...
      auto msg =  vescArray_.buildCommandMessage((vesc::vescChannelCommand){
              ...
              .commandValue = (useCurrentControl ? (double)MOTOR_NEUTRAL_ : signedMotorCommand)});
...
int RMProtocolObject::cycle_robot_mode() {
...
void RMProtocolObject::motors_control_loop(int sleeptime) {
  float linear_vel_target, angular_vel_target, rpm_FL, rpm_FR, rpm_BL, rpm_BR, left_hip_vel_target, right_hip_vel_target;
  ...
  while (true) {
    ...
    left_hip_vel_target = robotstatus_.cmd_left_hip_vel;
    right_hip_vel_target = robotstatus_.cmd_right_hip_vel;
    ...
    if (!estop_ &&
      ...
      auto velocities =
          rosco_control_->getMeasuredVelocities((Control::motor_data){
              .fl = rpm_FL, .fr = rpm_FR, .rl = rpm_BL, .rr = rpm_BR});
