...
motor_data computeRoscoManualWheelSpeeds(robot_velocities target_velocities,
                                        robot_hip_velocities target_hip_velocities,
                                        robot_geometry robot_geometry) {
  /* travel rate(m/s) */
  float left_travel_rate =
      target_velocities.linear_velocity -
  ...
  float right_travel_rate =
      target_velocities.linear_velocity +
      (0.5 * target_velocities.angular_velocity * robot_geometry.wheel_base);
  float rear_travel_rate = target_velocities.linear_velocity;
  float left_hip_rate = target_hip_velocities.left_hip_velocity * robot_geometry.intra_axle_distance; // femur length
  float right_hip_rate = target_hip_velocities.right_hip_velocity * robot_geometry.intra_axle_distance;
  ...
  float front_right_wheel_speed =
      (right_travel_rate / robot_geometry.wheel_radius) / RPM_TO_RADS_SEC;
  float rear_left_wheel_speed =
      ((rear_travel_rate + left_hip_rate) / robot_geometry.wheel_radius) / RPM_TO_RADS_SEC;
  float rear_right_wheel_speed =
      ((rear_travel_rate + right_hip_rate) / robot_geometry.wheel_radius) / RPM_TO_RADS_SEC;

  motor_data returnstruct = {front_left_wheel_speed, front_right_wheel_speed,
...
void RoscoManualController::setTrim(float left_trim, float right_trim) {
  left_trim_value_ = left_trim;
  right_trim_value_ = right_trim;
}
float RoscoManualController::getLeftTrim() {
  return left_trim_value_;
}
float RoscoManualController::getRightTrim() {
  return right_trim_value_;
}
...
RoscoManualController::RoscoManualController() {}
RoscoManualController::RoscoManualController(
    robot_motion_mode_t operating_mode, robot_geometry robot_geometry,
    float max_motor_duty, float min_motor_duty, float left_trim,
    float right_trim, float open_loop_max_wheel_rpm)
...
void RoscoManualController::initializePids() {
  pid_mutex_.lock();
  switch (operating_mode_) {
    case OPEN_LOOP:
      break;
...
void RoscoManualController::setAccelerationLimits(robot_velocities limits) {
  max_linear_acceleration_ = limits.linear_velocity;
  max_angular_acceleration_ = limits.angular_velocity;
}

robot_velocities RoscoManualController::getAccelerationLimits() {
  robot_velocities returnstruct;
  returnstruct.angular_velocity = max_angular_acceleration_;
  returnstruct.linear_velocity = max_linear_acceleration_;
  return returnstruct;
...
void RoscoManualController::setOperatingMode(
    robot_motion_mode_t operating_mode) {
  operating_mode_ = operating_mode;
  initializePids();
}

robot_motion_mode_t RoscoManualController::getOperatingMode() {
  return operating_mode_;
}

void RoscoManualController::setRobotGeometry(
    robot_geometry robot_geometry) {
  robot_geometry_ = robot_geometry;
}

robot_geometry RoscoManualController::getRobotGeometry() {
  return robot_geometry_;
}

void RoscoManualController::setPidGains(pid_gains pid_gains) {
  pid_gains_ = pid_gains;
}

pid_gains RoscoManualController::getPidGains() { return pid_gains_; }

void RoscoManualController::setMotorMaxDuty(float max_motor_duty) {
  max_motor_duty_ = max_motor_duty;
}
float RoscoManualController::getMotorMaxDuty() { return max_motor_duty_; }

void RoscoManualController::setMotorMinDuty(float min_motor_duty) {
  min_motor_duty_ = min_motor_duty;
}
float RoscoManualController::getMotorMinDuty() { return min_motor_duty_; }

void RoscoManualController::setOutputDecay(float geometric_decay) {
  geometric_decay_ = geometric_decay;
}
float RoscoManualController::getOutputDecay() { return geometric_decay_; }

void RoscoManualController::setOpenLoopMaxRpm(
    float open_loop_max_wheel_rpm) {
  open_loop_max_wheel_rpm_ = open_loop_max_wheel_rpm;
}
float RoscoManualController::getOpenLoopMaxRpm() {
  return open_loop_max_wheel_rpm_;
}

void RoscoManualController::setAngularScaling(
    angular_scaling_params angular_scaling_params) {
  angular_scaling_params_ = angular_scaling_params;
}

angular_scaling_params RoscoManualController::getAngularScaling() {
  return angular_scaling_params_;
}

motor_data RoscoManualController::computeMotorCommandsQuad_(
    motor_data target_wheel_speeds, motor_data current_wheel_speeds) {
  /* run pid, 1 per wheel */
  pid_mutex_.lock();
  pid_outputs fl_pid_output = pid_controller_fl_->runControl(
...
motor_data RoscoManualController::clipDutyCycles_(
    motor_data proposed_duties) {
  /* clip extreme duty cycles in either direction (positive or negative) */
  proposed_duties.fr =
      std::clamp(proposed_duties.fr, -max_motor_duty_, max_motor_duty_);
...
motor_data RoscoManualController::computeTorqueDistribution_(
    motor_data current_wheel_speeds, motor_data power_proposals) {
  /* right side */
  /* if both wheels are moving then ... */
...
robot_velocities RoscoManualController::getMeasuredVelocities(
    motor_data current_wheel_speeds) {
  return computeVelocitiesFromWheelspeeds(current_wheel_speeds,
                                          robot_geometry_);
}
...
motor_data RoscoManualController::runMotionControl(
  ...
  /* get target wheelspeeds from velocities */
  motor_data target_wheel_speeds =
      computeRoscoManualWheelSpeeds(velocity_commands, hip_commands, robot_geometry_);
